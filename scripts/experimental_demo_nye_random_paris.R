...
################################################################################
CORES = 10
SAVE_FILE_NAME = "experimental_demo_nye_random_pairs.rds"
################################################################################
# LOAD LIBRARIES
################################################################################
library(phangorn)
...
short = calculate_nye_distance_random_pairs(trees1 = long, trees2 = short, cores = CORES)
short_guides = calculate_nye_distance_random_pairs(trees1 = long, trees2 = short_guides, cores = CORES)
long_control = calculate_nye_distance_random_pairs(trees1 = long, trees2 = long_control, cores = CORES)

experimental_demo_nye =
  data.frame("short" = short,
             "short_guides" = short_guides,
             "long_control" = long_control)

setwd(dir.out)
saveRDS(experimental_demo_nye, file = SAVE_FILE_NAME, compress = TRUE)
