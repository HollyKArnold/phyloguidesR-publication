...
################################################################################
CORES = 10
SAVE_FILE_NAME = "experimental_demo_jrf_random_pairs.rds"
################################################################################
# LOAD LIBRARIES
################################################################################
library(phangorn)
...
short_jrf = calculate_jrf_distance_random_pairs(trees1 = long, trees2 = short, cores = CORES)
short_guides_jrf = calculate_jrf_distance_random_pairs(trees1 = long, trees2 = short_guides, cores = CORES)
long_control_jrf = calculate_jrf_distance_random_pairs(trees1 = long, trees2 = long_control, cores = CORES)

experimental_demo_jrf =
  data.frame("short" = short_jrf,
             "short_guides" = short_guides_jrf,
             "long_control" = long_control_jrf)

setwd(dir.out)
saveRDS(experimental_demo_jrf, file = SAVE_FILE_NAME, compress = TRUE)
