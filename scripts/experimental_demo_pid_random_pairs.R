...
################################################################################
CORES = 10
SAVE_FILE_NAME = "experimental_demo_pid_random_pairs.rds"
################################################################################
# LOAD LIBRARIES
################################################################################
library(phangorn)
...
short_pid = calculate_pid_distance_random_pairs(trees1 = long, trees2 = short, cores = CORES)
short_guides_pid = calculate_pid_distance_random_pairs(trees1 = long, trees2 = short_guides, cores = CORES)
long_control_pid = calculate_pid_distance_random_pairs(trees1 = long, trees2 = long_control, cores = CORES)


exp_demo_pid =
  data.frame("short" = short_pid,
             "short_guides" = short_guides_pid,
             "long_control" = long_control_pid)

setwd(dir.out)
saveRDS(exp_demo_pid, file = SAVE_FILE_NAME, compress = TRUE)
